...
namespace Naziki_Editor.Views
{
    public partial class AssetListControl : UserControl, IShortcutAware
    {
        ...
        {
            InitializeComponent();
            _messageBroker = AppServices.GetService<IMessageBroker>();
        ...
        {
            _ = ImportClipboardAssetsAsync();
        }

        private async Task ImportClipboardAssetsAsync()
        {
            ...
            if (Clipboard.ContainsFileDropList())
            {
                ...
                foreach (string sourceFile in files)
                {
                    if (File.Exists(sourceFile))
                    {
                        try
                        {
                            await AppServices.GetService<IProjectResourceService>()
                                .ImportAsync(Context, ProjectResourceKind.Asset, sourceFile);
                            hasChanged = true;
                        }
                        catch (Exception ex)
                        {
                            _dialogService.ShowErrorDialog(
                                $"导入文件失败：{Path.GetFileName(sourceFile)}\n原因：{ex.Message}",
                                "导入失败",
                                ex.ToString());
                        }
                    }
        ...
        private void TileBorder_PreviewMouseMove(object sender, MouseEventArgs e)
        {
                ...
                Math.Abs(diff.Y) > SystemParameters.MinimumVerticalDragDistance)
            {
                ...
                if (sender is Border border && border.DataContext is AssetItemViewModel asset)
                {
                    ...
                    {
                        // 包装拖拽数据：同时携带实体和素材信息
                        var dragData = new AssetDragPayload(asset, entity);
        ...
        {
            if ((sender as FrameworkElement)?.DataContext is not AssetItemViewModel asset) return;
            var entity = CreateEntityFromAsset(asset);
            if (entity != null) _messageBroker.Publish("CreateEventFromAsset", entity);
        }

        private void MenuLocate_Click(object sender, RoutedEventArgs e)
        {
            if ((sender as FrameworkElement)?.DataContext is not AssetItemViewModel asset ||
                !File.Exists(asset.FilePath)) return;
            Process.Start(new ProcessStartInfo("explorer.exe", $"/select,\"{asset.FilePath}\"")
            {
        ...
        /// </summary>
        private async void BtnImportAsset_Click(object sender, RoutedEventArgs e)
        {
