...
public static class CytoidCoreBuild
{
    ...
    public const string EditorHostDefineSymbol = "CYTOID_EDITOR_HOST";
    public const string DefaultWindowsPreviewRelativePath =
        "../../../../Runtime/OriginalPlayer/NazikiOriginalPlayer.exe";
    ...
    private const int MenuPriorityBuildWindowsPreview = 12;
    private const int MenuPriorityBuildWindowsPreviewRelease = 13;
    ...
    [MenuItem("Cytoid/Build Windows Editor Preview (Mono Development)", false, MenuPriorityBuildWindowsPreview)]
    public static void BuildWindowsEditorPreview()
    {
        QueueWindowsEditorPreviewBuild(developmentBuild: true);
    }

    [MenuItem("Cytoid/Build Windows Editor Preview (IL2CPP Release)", false, MenuPriorityBuildWindowsPreviewRelease)]
    public static void BuildWindowsEditorPreviewRelease()
    {
        QueueWindowsEditorPreviewBuild(developmentBuild: false);
    }

    private static void QueueWindowsEditorPreviewBuild(bool developmentBuild)
    {
        if (EditorPrefs.GetBool(PendingBuildActiveKey, false))
            throw new InvalidOperationException("A CytoidCoreBuild export is already in progress.");

        var output = ResolvePathUnderProjectRoot(DefaultWindowsPreviewRelativePath);
        SetPendingBuildState("windows-preview", output, developmentBuild, "");
        try
        {
    ...
    private static void ContinuePendingBuild()
    {
        ...
        try
        {
            ...
            else if (platform == "windows-preview")
            {
                RunWindowsEditorPreviewBuild(outputDir, packageFramework);
            }
            else
            {
    ...
    private static void RunWindowsEditorPreviewBuild(string outputExe, bool developmentBuild)
    {
        ...
        try
        {
            PlayerSettings.SetScriptingDefineSymbols(
                target,
                MergeDefineSymbols(previousDefines, new[] {EditorHostDefineSymbol}));
            PlayerSettings.SetScriptingBackend(
                target,
                developmentBuild ? ScriptingImplementation.Mono2x : ScriptingImplementation.IL2CPP);
            ...
            var options = new BuildPlayerOptions
            {
                scenes = PluginBuildScenes.Where(File.Exists).ToArray(),
            ...
            if (options.scenes.Length != PluginBuildScenes.Length)
                throw new FileNotFoundException("Windows Preview build scenes are incomplete.");

            var report = BuildPipeline.BuildPlayer(options);
            if (report.summary.result != BuildResult.Succeeded)
            {
                LogBuildReportErrors(report, "Windows Editor Preview");
                throw new Exception($"Windows Preview build failed: {report.summary.result}.");
            }
            Debug.Log(
                $"[CytoidCoreBuild] Windows Preview built at {Path.GetFullPath(outputExe)}\n"
                + $"  Define: {EditorHostDefineSymbol}\n"
                + $"  Backend: {(developmentBuild ? "Mono" : "IL2CPP")}\n"
