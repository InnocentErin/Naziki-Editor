...
public sealed class GameState
{
    ...
    public GameState(Game game, GameMode mode, HashSet<Mod> mods)
    {
        ...
        switch (mode)
        {
            case GameMode.Tier:
            {
                ...
                if ((Application.isEditor && game.EditorForceAutoMod) || GameEmbedMode.IsEditorPreview) Mods.Add(Mod.Auto);
    ...
    public void ResetPreviewAutoplayAt(float targetTime)
    {
        if (!GameEmbedMode.IsEditorPreview) return;
        var cleared = 0;
        foreach (var note in game.Chart.Model.note_list)
        {
            var judged = note.end_time <= targetTime;
            Judgements[note.id] = new NoteJudgement
            {
                IsJudged = judged,
                Grade = judged ? NoteGrade.Perfect : NoteGrade.None,
                Error = 0
            };
            if (judged) cleared++;
        }
        ...
        Combo = cleared;
        MaxCombo = cleared;
        Score = NoteCount == 0 ? 0 : Math.Min(1000000, 1000000.0 * cleared / NoteCount);
        accumulatedAccuracy = cleared;
        Accuracy = cleared == 0 ? 0 : 1;
