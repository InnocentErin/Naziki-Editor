...
public class Game : MonoBehaviour
{
    ...
    private bool previewPausedAtEnd;
#endif
...
#if !UNITY_EDITOR && !CYTOID_EDITOR_HOST
    ...
    protected virtual async void Start()
    {
        ...
        {
#if CYTOID_EDITOR_HOST && UNITY_STANDALONE_WIN
            await Initialize(!GameEmbedMode.IsEditorPreview);
#else
            await Initialize();
#endif
        }
    ...
    public async UniTask Initialize(bool startAutomatically = true)
    {
        ...
        if ((Application.isEditor && EditorForceAutoMod) || GameEmbedMode.IsEditorPreview)
        {
            mods.Add(Mod.Auto);
        }
        if (mode == GameMode.GlobalCalibration)
    ...
    protected virtual void Update()
    {
        ...
        {
            if (State.ClearCount >= Chart.Model.note_list.Count)
            {
#if CYTOID_EDITOR_HOST && UNITY_STANDALONE_WIN
                if (GameEmbedMode.IsEditorPreview)
                {
...
#if CYTOID_EDITOR_HOST && UNITY_STANDALONE_WIN
    /// <summary>
    /// Random-access evaluation used exclusively by the Naziki editor host. This bypasses
    /// gameplay side effects (SFX, vibration, completion and result reporting) and rebuilds
    /// the visible note window from the target time.
    /// </summary>
    public void PreviewEvaluateAt(float targetTime)
    {
        if (!GameEmbedMode.IsEditorPreview || !IsLoaded) return;
        var previewDuration = Mathf.Max(MusicLength, ChartLength);
        targetTime = Mathf.Clamp(targetTime, 0, previewDuration);
        if (targetTime < previewDuration - 0.001f)
        ...
        foreach (var note in ObjectPool.SpawnedNotes.Values.ToList())
            note?.Collect();
        foreach (var dragLine in ObjectPool.SpawnedDragLines.Values.ToList())
            dragLine?.Collect();

        var notes = Chart.Model.note_map;
        Chart.CurrentEventId = Chart.Model.event_order_list.FindIndex(it => it.time >= targetTime);
        if (Chart.CurrentEventId < 0) Chart.CurrentEventId = Chart.Model.event_order_list.Count;
        Chart.CurrentPageId = Chart.Model.page_list.FindIndex(it => it.end_time > targetTime);
        if (Chart.CurrentPageId < 0) Chart.CurrentPageId = Chart.Model.page_list.Count;
        Chart.CurrentNoteId = 0;
        while (Chart.CurrentNoteId < notes.Count &&
               notes[Chart.CurrentNoteId].intro_time - 1f < targetTime)
            Chart.CurrentNoteId++;

        foreach (var noteModel in Chart.Model.note_list)
        {
            if (noteModel.intro_time - 1f >= targetTime || noteModel.end_time + 1f <= targetTime)
                continue;
            ObjectPool.SpawnNote(noteModel);
        }

        State.ResetPreviewAutoplayAt(targetTime);
        if (Storyboard != null)
        {
    ...
    public async UniTask PreviewAdvanceExternalClock(float targetTime)
    {
        ...
        while (Chart.CurrentNoteId < notes.Count && notes[Chart.CurrentNoteId].intro_time - 1f < Time)
        {
            var model = notes[Chart.CurrentNoteId];
            if ((NoteType)model.type == NoteType.DragHead || (NoteType)model.type == NoteType.CDragHead)
            {
                var id = Chart.CurrentNoteId;
                while (notes[id].next_id > 0)
                {
    ...
    public void PreviewPlayFromCurrentTime()
    {
        if (!GameEmbedMode.IsEditorPreview || !IsLoaded) return;
        var previewDuration = Mathf.Max(MusicLength, ChartLength);
        if (Time >= previewDuration - 0.001f)
            PreviewEvaluateAt(0);
        previewPausedAtEnd = false;
        if (Storyboard != null) Storyboard.Renderer.IsRandomAccessEvaluation = false;
        AudioListener.pause = false;
        MusicStartedTimestamp = Music.PlayFrom(Time);
        GameStartedOrResumedTimestamp = UnityEngine.Time.realtimeSinceStartup;
        State.IsStarted = true;
        State.IsPlaying = true;
    }
    ...
    public float PreviewDuration => Mathf.Max(MusicLength, ChartLength);

    private void PreviewPauseAtEnd()
    {
        if (!GameEmbedMode.IsEditorPreview || !IsLoaded || previewPausedAtEnd) return;
        previewPausedAtEnd = true;
        var duration = PreviewDuration;
        PreviewEvaluateAt(duration);
        Music.Pause();
        AudioListener.pause = true;
        State.IsPlaying = false;
