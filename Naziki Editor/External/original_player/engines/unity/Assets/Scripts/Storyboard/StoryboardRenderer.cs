...
namespace Cytoid.Storyboard
{
    public class StoryboardRenderer
    {
        ...
        public bool IsRandomAccessEvaluation { get; set; }
        ...
        public void OnGameUpdate(Game _)
        {
            ...
            foreach (var type in updateOrder)
            {
                ...
                foreach (var renderer in renderers)
                {
                    ...
                    if (fromState == null)
                    {
                        if (IsRandomAccessEvaluation) renderer.Clear();
                        continue;
                    }
        ...
        public void OnTrigger(Trigger trigger)
        {
            ...
            if (trigger.Destroy != null)
            {
                ...
                {
                    if (IsRandomAccessEvaluation &&
                        ComponentRenderers.TryGetValue(id, out var renderer))
                        renderer.Clear();
                    else
        ...
        {
            await SpawnObjectByIdAsync(id);
        }

        public async UniTask SpawnObjectByIdAsync(string id)
        {
        ...
        public void DestroyObjectsById(string id)
        {
            ...
            {
                it.Dispose();
                foreach (var renderers in TypedComponentRenderers.Values)
                    renderers.Remove(it);
            });
            ComponentRenderers.Remove(id);
        }
